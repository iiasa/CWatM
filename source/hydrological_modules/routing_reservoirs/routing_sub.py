...
def postorder(dirUp,catchment,node,catch):

     if dirUp[node] != []:
        postorder(dirUp,catchment,dirUp[node][0],catch)
        catchment[dirUp[node][0]]=catch
        dirDown.append(dirUp[node][0])
        if len(dirUp[node])>1:
            postorder(dirUp,catchment,dirUp[node][1],catch)
            catchment[dirUp[node][1]]=catch
            dirDown.append(dirUp[node][1])
            if len(dirUp[node])>2:
               postorder(dirUp,catchment,dirUp[node][2],catch)
               catchment[dirUp[node][2]]=catch
               dirDown.append(dirUp[node][2])
               if len(dirUp[node])>3:
                postorder(dirUp,catchment,dirUp[node][3],catch)
                catchment[dirUp[node][3]]=catch
                dirDown.append(dirUp[node][3])
                if len(dirUp[node])>4:
                 postorder(dirUp,catchment,dirUp[node][4],catch)
                 catchment[dirUp[node][4]]=catch
                 dirDown.append(dirUp[node][4])
                 if len(dirUp[node])>5:
                  postorder(dirUp,catchment,dirUp[node][5],catch)
                  catchment[dirUp[node][5]]=catch
                  dirDown.append(dirUp[node][5])
                  if len(dirUp[node])>6:
                   postorder(dirUp,catchment,dirUp[node][6],catch)
                   catchment[dirUp[node][6]]=catch
                   dirDown.append(dirUp[node][6])
                   if len(dirUp[node])>7:
                    postorder(dirUp,catchment,dirUp[node][7],catch)
...
def dirDownstream(dirUp,lddcomp):
    ...
    for pit in xrange(maskinfo['mapC'][0]):
        if lddcomp[pit]==5:
            j+=1
            postorder(dirUp,catchment,pit,j)
            dirDown.append(pit)
            catchment[pit]=j
...
def catchment1(dirUp, points):
    ...
    for cell in xrange(maskinfo['mapC'][0]):
        ...
        if (j > 0) and (subcatch[cell] < 1):
            postorder(dirUp, subcatch, cell, j)
            subcatch[cell] = j
...
def defLdd2(ldd):
    ...
    dirDown, catchment = dirDownstream(dirUp, lddCompress)
