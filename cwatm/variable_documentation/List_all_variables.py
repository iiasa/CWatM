...
def extract_selfvar(module_name):
    ...
    for ii in range(len(aa)): # for each line in the Python code
        ...
        bb = aa[ii]
        bb = bb.lstrip()  # Removing space at the begining
            ...
            while indexselfvar != -1:
                ...
                if indexselfvar != -1:  # there is at least one self.var in the line
                    ...
                    for ww in range(indexselfvar, len(bb_temp)+1):
                        if ww == len(bb_temp) or bb_temp[ww] == ':' or bb_temp[ww] == ',' or bb_temp[ww] == '(' or bb_temp[ww] == ')'\
                                or bb_temp[ww] == ' ' or bb_temp[ww] == '=' or bb_temp[ww] == '*' or bb_temp[ww] == '+' or bb_temp[ww] == '-'\
                                or bb_temp[ww] == '/' or bb_temp[ww] == '[' or bb_temp[ww] == ']' or bb_temp[ww] == '"' or bb_temp[ww:ww+5] == '.appe'\
                                or bb_temp[ww:ww+5] == '.copy' or bb_temp[ww:ww+5] == '.ravel' or bb_temp[ww:ww+5] == '<' or bb_temp[ww:ww+5] == '>'\
...
#PB New folders and removed the 2nd and 3rd definition of this
...
# folders = ['hydrological_modules', 'hydrological_modules/routing_reservoirs',
#           'hydrological_modules/groundwater_modflow', 'management_modules']
...
for folders_paths in folders:
    ...
    for ll in range(len(python_modules)): 
        if python_modules[ll][-2:] == 'py' and python_modules[ll][-2:] != 'List_all_variables':  # if the file is a Python module
            ...
            for ii in range(len(variable_names_list)):  # For each variable
                ...
                if variable_names_list[ii] not in Dict_AllVariables:  # if the self.var is not already defined
                    ...
                    if name_module_python in Dict_AllVariables[variable_names_list[ii]]:  # if the variable is already defined in this module
                        if first_definition[ii] ==1:
...
for ir in range(1,rows):
    name_complete = 'self.var.' + sheet.cell_value(ir, 1)
    var_name_in_excel.append(name_complete)
    var_description_in_excel.append(sheet.cell_value(ir, 5))
    var_unit_in_excel.append(sheet.cell_value(ir, 4))
        ...
        if excel_name.startswith(name_var): # the excel variable is probably the same as namevar
            ...
            if aa == 0:  # we have the best match, we can add the corresponding description
                if 'description' in Dict_AllVariables[name_var]:  # we already fill the description
                    if Dict_AllVariables[name_var]['description'] == '':  # we replace by a better description than empty text
...
for ii in Dict_AllVariables:
    print('ii', ii)
    #print(str(Dict_AllVariables[ii]['description']))
    #print(str(Dict_AllVariables[ii]['unit']))
    print(str(Dict_AllVariables[ii]))
...
print('Modifying CWATM modules to add self.var informations')

# PB defined before
#folders = ['hydrological_modules', 'hydrological_modules/routing_reservoirs','hydrological_modules/groundwater_modflow', 'management_modules']
for folders_paths in folders:
    path = str(folders_paths)
    python_modules = os.listdir(path)
    for ll in range(len(python_modules)):
        if python_modules[ll][-2:] == 'py':
            ...
            if len(var_name_in_module) > 0:
                ...
                added_description = added_description + lead + '{:{x}.{x}}'.format('Variable [self.var]',x=col1) + between + '{:{x}.{x}}'.format('Description', x=col2) + between +\
                ...
                for module_line in range(len(lines)):
                    ...
                    if lines[module_line].startswith('    """') and begin_outcommented_lines != -1 and class_index != -1 :
...
for ir in range(1, rows):
    ...
    var_description_in_excel.append(sheet.cell_value(ir, 5))
    var_unit_in_excel.append(sheet.cell_value(ir, 4))
    var_standardname_in_excel.append(sheet.cell_value(ir, 3))
